...
# Preprocessing Page
    ...
    if st.session_state.proses == 'false': # Bukan About Model - Preprocessing
        ...
        df['stemming'] = df['stopword'].apply(stemming_indonesia)

        if st.session_state.tipe == 'csv': # CSV
            if st.session_state.predict == 'false': # Evaluasi
                st.dataframe(df[['content','lower_case','clean_1', 'clean_2', 'normalized', 'tokens', 'stopword', 'stemming','sentimen']], hide_index=True)
            else: # Prediksi
                st.dataframe(df[['content','lower_case','clean_1', 'clean_2', 'normalized', 'tokens', 'stopword', 'stemming']], hide_index=True)
...
# Output Page
    ...
    if st.session_state.proses == 'false': # Bukan About Model - Output
        ...
        word2vec_model = Word2Vec.load(word2vec)
